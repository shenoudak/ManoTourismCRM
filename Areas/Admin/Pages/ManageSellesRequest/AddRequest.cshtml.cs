...
namespace ManoTourism.Areas.Admin.Pages.ManageSellesRequest
{
        ...
        public async Task<IActionResult> OnPost(string OneWayCheck)
        {
            ...
            try
            {
                ...
                if (await _userManager.IsInRoleAsync(user, "selles"))
                {
                    ...
                    var sellesUser = _context.Sales.Where(e => e.SalesId == user.EntityId).FirstOrDefault();
                    if (sellesUser == null)
                    {
                        return Redirect("/Identity/account/login");
                    }
                    var emp = _context.Employees.Where(e => e.EmployeeId == sellesUser.EmployeeId).FirstOrDefault();
                    if (emp != null)
                    {
                        AddNewRequest.SharedEmployeeeName = emp.EmployeeName;
                        AddNewRequest.SharedEmployeeeId = emp.EmployeeId;
                    }
                }
            ...
            return Redirect("/Admin/ManageSellesRequest/Index");
        }
