...
namespace ManoTourism.Areas.Admin.Pages.ManageInsuranceRequests
{
    public class IndexModel : PageModel
    {
        ...
        public static int staticInsuranceId { get; set; }
        ...
        public async Task<IActionResult> OnGet(int InsuranceId)
        {
            ...
            try
            {
                var Insurance = _context.Insurances.Where(e => e.InsuranceId == InsuranceId).FirstOrDefault();
                if (Insurance == null)
                {
                    return Redirect("/Admin/PageNotFound");
                }
                url = $"{this.Request.Scheme}://{this.Request.Host}";
                staticInsuranceId = InsuranceId;
            }
            catch (Exception ex)
            {
        ...
        public async Task<JsonResult> OnPostAsync()
        {
            ...
            var recordsTotal = _context.Requests.Where(e => e.IsDeleted == false&&e.EntityId==staticInsuranceId && e.ManoEntityTypeId == 3).Count();
            
            var customersQuery = _context.Requests.Include(e => e.Country).Include(e => e.VisaRequestStatus).Include(e => e.Nationality).Include(e => e.ManoEntityType).Include(e => e.CompanyMarkting).Where(e => e.IsDeleted == false && e.EntityId == staticInsuranceId&&e.ManoEntityTypeId==3).Select(i => new
            {
