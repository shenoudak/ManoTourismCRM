...
namespace ManoTourism.Areas.Admin.Pages.EmployeeSharedSellers
{
    ...
    public class SellersRequestsModel : PageModel
    {
        ...
        public SellersRequestsModel(ManoContext context, IToastNotification toastNotification, UserManager<ApplicationUser> userManager, ApplicationDbContext db)
        {
            _context = context;
        ...
        public async Task<IActionResult> OnGet(int userId)
        {
            ...
            var CurrEmp = await _userManager.GetUserAsync(User);
            if (CurrEmp == null)
            {
                return Redirect("/identity/account/login");
            }
            
            var Employee = _context.Employees.Where(e => e.EmployeeEmail == CurrEmp.Email).FirstOrDefault();
            if (Employee == null)
            {
                return Redirect("/Identity/account/login");
            }
        ...
        public async Task<JsonResult> OnPostAsync()
        {
            ...
            var recordsTotal = _context.Requests.Where(e => e.IsDeleted == false && e.UserId == UserId&&e.EmployeeId== EmployeeId).Count();
            var customersQuery = _context.Requests.Include(e => e.Country).Include(e => e.VisaRequestStatus).Include(e => e.Nationality).Include(e => e.ManoEntityType).Include(e => e.CompanyMarkting).Where(e => e.IsDeleted == false && e.UserId == UserId&& e.SharedEmployeeeId == EmployeeId).Select(i => new
            {
