...
namespace ManoTourism.Areas.Admin.Pages.ManageEstCompanyRequests
{
    public class IndexModel : PageModel
    {
        ...
        public static int staticEsCompanyId { get; set; }
        ...
        public async Task<IActionResult> OnGet(int EstablishingCompanyId)
        {
            ...
            try
            {
                ...
                if (aleadyAuthorized)
                {
                    var EsCompany = _context.EstablishingCompanies.Where(e => e.EstablishingCompanyId == EstablishingCompanyId).FirstOrDefault();
                    if (EsCompany == null)
                    {
                        return Redirect("/Admin/PageNotFound");
                    }
                    url = $"{this.Request.Scheme}://{this.Request.Host}";
                    staticEsCompanyId = EstablishingCompanyId;
        ...
        public async Task<JsonResult> OnPostAsync()
        {
            ...
            var recordsTotal = _context.Requests.Where(e => e.IsDeleted == false&&e.EntityId==staticEsCompanyId&& e.ManoEntityTypeId == 4).Count();

            var customersQuery = _context.Requests.Include(e => e.Country).Include(e => e.VisaRequestStatus).Include(e => e.Nationality).Include(e => e.ManoEntityType).Include(e => e.CompanyMarkting).Where(e => e.IsDeleted == false && e.EntityId == staticEsCompanyId && e.ManoEntityTypeId == 4).Select(i => new
            {
            ...
            if (!string.IsNullOrWhiteSpace(searchText))
            {
                customersQuery = customersQuery.Where(s =>
                    ...
                    s.EntityTitleAr.ToUpper().Contains(searchText)
                );
            }
