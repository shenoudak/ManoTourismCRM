...
{
    public class NewRequestsModel : PageModel
    {
        ...
        public NewRequestsModel(ManoContext context, IToastNotification toastNotification, UserManager<ApplicationUser> userManager)
        {
            _context = context;
            ...
            try
            {
                ...
                if (roleName.FirstOrDefault() == "admin" || roleName.FirstOrDefault() == "employee")
                {
                    ...
                    if (roleName.FirstOrDefault() == "employee")
                    {
                        ...
                        {
                            bool isAuthoried = _context.AssignEmployeeRoles.Any(e => e.EmployeeId == employee.EmployeeId && e.EmployeeRoleId == (int)EmpRoles.NewRequests);
                            if (isAuthoried)
                            {
                                aleadyAuthorized = true;
                            }
        ...
        public async Task<JsonResult> OnPostAsync()
        {
            ...
            var recordsTotal = _context.Requests.Where(e => e.IsDeleted == false && e.VisaRequestStatusId == 1).Count();
            ...
            var customersQuery = _context.Requests.Include(e => e.Country).Include(e => e.VisaRequestStatus).Include(e => e.Nationality).Include(e => e.ManoEntityType).Include(e => e.CompanyMarkting).Where(e => e.IsDeleted == false && e.VisaRequestStatusId == 1).Select(i => new
            {
        ...
        public async Task<IActionResult> OnPostAssignRequest()
        {

            try
            {
                var model = _context.Requests.Where(c => c.RequestId == assignRequestVM.VisaRequestId).FirstOrDefault();
                if (model == null)
                {
                    _toastNotification.AddErrorToastMessage("Object Not Found");

                    return Redirect("/Admin/ManageRequests/NewRequests");
                }




                model.EmployeeId = assignRequestVM.EmployeeId;
                model.AssignedDateToEmployee = DateTime.Now;

                model.VisaRequestStatusId = 2;
                ...
                _toastNotification.AddSuccessToastMessage("Employee Assigned To Request successfully");
