...
namespace ManoTourism.Areas.Admin.Pages.ManageTicketRequests
{
    public class IndexModel : PageModel
    {
        ...
        public static int staticTicketId { get; set; }
        ...
        public async Task<IActionResult> OnGet(int TicketId)
        {
            ...
            try
            {
                ...
                if (aleadyAuthorized)
                {
                    var Ticket = _context.Tickets.Where(e => e.TicketId == TicketId).FirstOrDefault();
                    if (Ticket == null)
                    {
                        return Redirect("/Admin/PageNotFound");
                    }
                    url = $"{this.Request.Scheme}://{this.Request.Host}";
                    staticTicketId = TicketId;
        ...
        public async Task<JsonResult> OnPostAsync()
        {
            ...
            var recordsTotal = _context.Requests.Where(e => e.IsDeleted == false && e.EntityId == staticTicketId && e.ManoEntityTypeId == 6).Count();

            var customersQuery = _context.Requests.Include(e => e.Country).Include(e => e.VisaRequestStatus).Include(e => e.Nationality).Include(e => e.ManoEntityType).Include(e => e.CompanyMarkting).Where(e => e.IsDeleted == false && e.EntityId == staticTicketId && e.ManoEntityTypeId == 6).Select(i => new
            {
